...
class KelompokController extends Controller
{
    ...
    public function index($id){
        ...
        $result_peserta = User::find($id)->peserta;
    ...
    public function store(Request $request){
        ...
        return redirect('/kelompok/'.$request['kd_peserta']);
    }
    ...
    public function invitesearch(Request $request){
        ...
        //validasi input kode
        $this->validate($request,[
            'kode' => 'required|min:20',
            ]);

        //nyari peserta berdasarkan kode invite(buat input id peserta)
        $result_peserta = DB::table('pesertas')->select()->where('kode_invite','=',$request['kode'])->get();

        //print_r($result_peserta);
        //die();
        ...
        $result_ketua = DB::table('pesertas')->select()->where('id','=',Auth::user()->id)->get();
        if(!$result_ketua){
            dd('not found');
        }
        //dd($result_ketua);
        ...
        if(!$result_anggota){
            ...
            if ($result_peserta[0]->asal_sekolah != $result_ketua[0]->asal_sekolah) {
                ...
                $message = 4;
            } else if ($result_peserta[0]->jenis_peserta != "Angota") {
                $message = 5;
            }
            else {
                //input ke anggota
        ...
        return redirect('/kelompok/'.Auth::user()->id)->with(['msg'=>$message]);
